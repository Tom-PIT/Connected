...
namespace TomPIT.UI.Theming.Parser
{
	...
	public class LessParser
    {
        ...
        public IImporter Importer
        {
            ...
            {
                _importer = value;
                _importer.Parser = () => new LessParser(Tokenizer.Optimization, Stylizer, _importer)
                {
        ...
        public LessParser(bool debug)
            : this(defaultOptimization, debug)
        {
        }

        public LessParser(int optimization)
            : this(optimization, new PlainStylizer(), new Importer(), defaultDebug)
        {
        }

        public LessParser(int optimization, bool debug)
            : this(optimization, new PlainStylizer(), new Importer(), debug)
        {
        }

        public LessParser(IStylizer stylizer, IImporter importer)
            : this(defaultOptimization, stylizer, importer, defaultDebug)
        {
        }

        public LessParser(IStylizer stylizer, IImporter importer, bool debug)
            : this(defaultOptimization, stylizer, importer, debug)
        {
        }

        public LessParser(int optimization, IStylizer stylizer, IImporter importer)
            : this(optimization, stylizer, importer, defaultDebug)
        {
        }

        public LessParser(int optimization, IStylizer stylizer, IImporter importer, bool debug)
        {
            Stylizer = stylizer;
            Importer = importer;
            Debug = debug;
