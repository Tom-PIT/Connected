...
internal static class MicroServiceCompiler
{
	...
	private static SyntaxTree? LoadAdditionalFile(Guid microService, Guid token)
	{
		var text = Tenant.GetService<IComponentService>().SelectText(microService, token, BlobTypes.AutoGenerated);
		var info = Tenant.GetService<IComponentService>().SelectTextInfo(microService, token, BlobTypes.AutoGenerated);
