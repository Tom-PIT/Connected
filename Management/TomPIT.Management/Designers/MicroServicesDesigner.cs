...
namespace TomPIT.Management.Designers
{
	internal class MicroServicesDesigner : CollectionDesigner<MicroServicesElement>
	{
		...
		private IDesignerActionResult CreateMicroService()
		{
			...
			Environment.Context.Tenant.GetService<IMicroServiceManagementService>().Insert(id, name, DomQuery.Closest<IResourceGroupScope>(Owner).ResourceGroup.Token, template, MicroServiceStages.Any, null);
