    ...
    public class DeploymentDesigner : DeploymentDesignerBase<MarketplaceElement>
    {
        ...
        public List<IPackageStateDescriptor> Packages
        {
            get
            {
                ...
                {
                    var published = Environment.Context.Tenant.GetService<IDeploymentService>().QueryPackages(Plan.Token);
                    _packages = new();

                    if (published is not null)
                    {
                        foreach (var package in published)
                        {
                            _packages.Add(new PackageStateDescriptor
                            {
                                ...
                                ShellBuild = package.ShellBuild,
                                ShellMajor = package.ShellMajor,
                                ShellMinor = package.ShellMinor,
                                ShellRevision = package.ShellRevision,
                                Title = package.Title,
                                Token = package.Token,
                                Url = package.Url,
                                Website = package.Website,
                                State = ResolveState(package.Token, package.Service)
                            });
                        }
                    }
                }
