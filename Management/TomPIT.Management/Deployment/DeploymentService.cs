...
namespace TomPIT.Management.Deployment
{
    internal class DeploymentService : IDeploymentService
    {
        ...
        public void PublishPackage(Guid microService)
        {
            ...
            if (ms != null)
                Connection.GetService<IMicroServiceManagementService>().Update(ms.Token, ms.Name, ms.Status, ms.Template, ms.ResourceGroup, ms.Package, ms.UpdateStatus, CommitStatus.Synchronized);
        }
        ...
        public List<Guid> CheckForUpdates(List<IMicroService> microServices)
        {
            ...
            foreach (var microService in microServices)
            {
                a.Add(new JObject
                {
                    {"package", microService.Token },
                    {"version", microService.Version }
                });
            }

            return Connection.Post<List<Guid>>(u, e);
        }
    }
}
