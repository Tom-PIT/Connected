...
namespace TomPIT.Management.ComponentModel
{
	internal class MicroServiceManagementService : TenantObject, IMicroServiceManagementService
	{
		...
		{
			var components = Tenant.GetService<IComponentDevelopmentService>().Query(microService);

			foreach (var i in components)
				Tenant.GetService<IComponentDevelopmentService>().Delete(i.Token, true);
		...
		private void DeleteFolder(FolderModel model)
		{
			...
			Tenant.GetService<IComponentDevelopmentService>().DeleteFolder(model.Folder.MicroService, model.Folder.Token);
		}
