...
namespace TomPIT.Management.ComponentModel
{
	internal class MicroServiceManagementService : TenantObject, IMicroServiceManagementService
	{
		...
		{
			Instance.SysProxy.Management.MicroServices.Insert(token, name, resourceGroup, template, supportedStages, version);

			if (Tenant.GetService<IMicroServiceService>() is IMicroServiceNotification svc)
				svc.NotifyChanged(this, new MicroServiceEventArgs(token));
		}

		public void Update(Guid microService, string name, MicroServiceStages supportedStages, Guid template, Guid resourceGroup)
		{
			Instance.SysProxy.Management.MicroServices.Update(microService, name, resourceGroup, template, supportedStages);
