...
namespace TomPIT.Design
{
	public class UserDesigner : DomDesigner<UserElement>
	{
		...
		{
			SysContext.GetService<IUserManagementService>().ResetPassword(User.Token);
		...
		{
			SysContext.GetService<IUserManagementService>().ChangeAvatar(User.Token, null, null, null);
		...
		private IDesignerActionResult Upload()
		{
			...
			using (var s = new MemoryStream())
			{
				...
				SysContext.GetService<IUserManagementService>().ChangeAvatar(User.Token, buffer, file.ContentType, System.IO.Path.GetFileName(file.FileName));
			};
		...
		public string AvatarUrl
		{
			get
			{
				...
				var b = SysContext.GetService<IStorageService>().Select(User.Avatar);
