...
namespace TomPIT.Design
{
	internal class EndpointsDesigner : CollectionDesigner<EndpointsElement>
	{
		...
		protected override IDesignerActionResult Remove(JObject data)
		{
			...
			SysContext.GetService<IInstanceEndpointManagementService>().Delete(endpoint.Token);
		...
		protected override IDesignerActionResult Clear(JObject data)
		{
			foreach (var i in Owner.Existing)
				SysContext.GetService<IInstanceEndpointManagementService>().Delete(i.Token);
		...
		private IDesignerActionResult CreateEndpoint()
		{
			...
			var name = SysContext.GetService<INamingService>().Create("Endpoint", existing.Select(f => f.Name), true);

			var id = SysContext.GetService<IInstanceEndpointManagementService>().Insert(name, Net.InstanceType.Application, string.Empty, string.Empty, Net.InstanceStatus.Disabled, Net.InstanceVerbs.All);
			var endpoint = SysContext.GetService<IInstanceEndpointService>().Select(id);
