...
namespace TomPIT.Design
{
	internal class RolesDesigner : CollectionDesigner<RolesElement>
	{
		...
		protected override IDesignerActionResult Remove(JObject data)
		{
			...
			SysContext.GetService<IRoleManagementService>().Delete(user.Token);
		...
		protected override IDesignerActionResult Clear(JObject data)
		{
			foreach (var i in Owner.Existing)
				SysContext.GetService<IRoleManagementService>().Delete(i.Token);
		...
		private IDesignerActionResult CreateRole()
		{
			...
			var name = SysContext.GetService<INamingService>().Create("Role", existing.Select(f => f.Name), true);

			var id = SysContext.GetService<IRoleManagementService>().Insert(name);
			var role = SysContext.GetService<IRoleService>().Select(id);
