...
namespace TomPIT.DataProviders.Modbus
{
	...
	internal class ModbusClient : IDisposable
	{
		...
		/// <summary>
		/// Converts two ModbusRegisters to Float - Example: ModbusClient.ConvertRegistersToFloat(modbusClient.ReadHoldingRegisters(19,2))
		/// </summary>
		...
		public static long ConvertRegistersToLong(int[] registers)
		{
			...
			byte[] bytes0 = BitConverter.GetBytes(reg0);
			byte[] bytes1 = BitConverter.GetBytes(reg1);
			byte[] bytes2 = BitConverter.GetBytes(reg2);
			byte[] bytes3 = BitConverter.GetBytes(reg3);
			...
			return BitConverter.ToInt64(doubleBytes, 0);
		}
		public static int ConvertRegistersToLong(int[] registers, RegisterOrder registerOrder)
		{
		...
		/// <summary>
		/// Converts float to two ModbusRegisters - Example:  modbusClient.WriteMultipleRegisters(24, ModbusClient.ConvertFloatToTwoRegisters((float)1.22));
		/// </summary>
		...
		/// <param name="registerOrder">Desired Word Order (Low Register first or High Register first</param>
		/// <returns>Register values</returns>
		public static int[] ConvertFloatToRegisters(float value, RegisterOrder registerOrder)
		{
			int[] registerValues = ConvertFloatToRegisters(value);
		...
		/// <returns>Register values</returns>
		public static int[] ConvertIntToRegisters(int doubleValue)
		{
		...
		/// <param name="registerOrder">Desired Word Order (Low Register first or High Register first</param>
		/// <returns>Register values</returns>
		public static int[] ConvertIntToRegisters(int doubleValue, RegisterOrder registerOrder)
		{
			int[] registerValues = ConvertIntToRegisters(doubleValue);
			int[] returnValue = registerValues;
		...
		/// <summary>
		/// Converts 64 Bit Value to four ModbusRegisters Registers - Registers can be swapped
		/// </summary>
		/// <param name="value">Long value which has to be converted into four registers</param>
		/// <param name="registerOrder">Desired Word Order (Low Register first or High Register first</param>
		/// <returns>Register values</returns>
		public static int[] ConvertLongToRegisters(long value)
		{
		...
		/// <param name="registerOrder">Desired Word Order (Low Register first or High Register first</param>
		/// <returns>Register values</returns>
		public static int[] ConvertLongToRegisters(long value, RegisterOrder registerOrder)
		{
			int[] registerValues = ConvertLongToRegisters(value);
			int[] returnValue = registerValues;

			if (registerOrder == RegisterOrder.HighLow)
				returnValue = new Int32[] { registerValues[3], registerValues[2], registerValues[1], registerValues[0] };
		...
		public object[] ReadHoldingRegisters(int startingAddress, int quantity, ModbusDataType dataType, RegisterOrder registerOrder)
		{
			int quantityToRead = quantity;
			if (dataType == ModbusDataType.Long | dataType == ModbusDataType.ULong | dataType == ModbusDataType.Float)
				quantityToRead = quantity * 2;
			if (dataType == ModbusDataType.Float)
				quantityToRead = quantity * 4;
			int[] response = this.ReadHoldingRegisters(startingAddress, quantityToRead);
			switch (dataType)
			{
				case ModbusDataType.Short:
					return response.Cast<object>().ToArray();
				default:
					return response.Cast<object>().ToArray();
			}
		}
		...
		public static ModbusClient Create(ModbusConnectionString connectionString)
		{
			var connection = string.IsNullOrWhiteSpace(connectionString.SerialPort)
				? new ModbusClient(connectionString.IpAddress, connectionString.Port)
				: new ModbusClient(connectionString.SerialPort);


			if (connectionString.Device > 0)
				connection.UnitIdentifier = connectionString.Device;
