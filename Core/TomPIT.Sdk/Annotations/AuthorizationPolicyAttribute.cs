...
namespace TomPIT.Annotations
{
    ...
    public abstract class AuthorizationPolicyAttribute : Attribute
    {
        ...
        {
            return AsyncUtils.RunSync(() => TryAuthorizeAsync(context, instance));
        }

        public async Task<bool> TryAuthorizeAsync(IMiddlewareContext context, object instance)
        {
            try
            {
                await AuthorizeAsync(context, instance);
                return true;
            }
            catch
            {
        ...
        {
            AsyncUtils.RunSync(() => AuthorizeAsync(context, instance));
        }

        public async Task AuthorizeAsync(IMiddlewareContext context, object instance)
        {
            ReflectionExtensions.SetPropertyValue(Model, nameof(Model.Context), context);
            ...
            if (await AuthorizeGrant(grantAttributes, AuthorizationMiddlewareStage.Before))
                return;
            ...
            if (messages.Count > 0)
            {
                if (await AuthorizeGrant(grantAttributes, AuthorizationMiddlewareStage.After))
                    return;
        ...
        {
            AsyncUtils.RunSync(() => OnAuthorizeAsync(results));
        }

        protected virtual async Task OnAuthorizeAsync(List<PolicyAuthorizationResult> results)
        {
        ...
        {
            return AsyncUtils.RunSync(() => AuthorizeAnyAsync(primaryKey, user, claims));
        }
        protected async Task<bool> AuthorizeAnyAsync(object primaryKey, Guid user, params object[] claims)
        {
            ...
            foreach (var claim in claims)
            {
                ...
                if (await Context.Services.Authorization.AuthorizeAsync(criteria, primaryKey.ToString(), PermissionDescriptor, user))
                    return true;
            }
        ...
        {
            return AsyncUtils.RunSync(() => AuthorizeAnyAsync(primaryKey, claims));
        }

        protected async Task<bool> AuthorizeAnyAsync(object primaryKey, params object[] claims)
        {
            var user = Guid.Empty;
            ...
            return await AuthorizeAnyAsync(primaryKey, user, claims);
        }
        ...
        protected async Task<bool> AuthorizeAllAsync(object primaryKey, Guid user, params object[] claims)
        {
            if (primaryKey == null || string.IsNullOrWhiteSpace(primaryKey.ToString()))
                throw new RuntimeException(GetType().Name, SR.ErrPrimaryKeyNull);
        ...
        {
            return AsyncUtils.RunSync(() => AuthorizeAllAsync(primaryKey, claims));
        }

        protected async Task<bool> AuthorizeAllAsync(object primaryKey, params object[] claims)
        {
            var user = Guid.Empty;
            ...
            return await AuthorizeAllAsync(primaryKey, user, claims);
        }
        ...
        private async Task<bool> AuthorizeGrant(List<AuthorizationPolicyAttribute> attributes, AuthorizationMiddlewareStage middleware)
        {
            ...
            foreach (var target in targets)
            {
                ...
                {
                    target.Model = Model;
                    await target.AuthorizeAsync(Context, Model.Proxy);
                    return true;
                }
