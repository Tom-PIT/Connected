...
namespace TomPIT.Runtime.ApplicationContextServices
{
	...
	/// </summary>
	internal class Database : ApplicationContextClient
	{
		public Database(IApplicationContext context) : base(context)
		{
		}
		/// <summary>
		...
		protected IConnection CreateConnection(IApplicationContext context, IExecutionContext sender, Guid id, string authorityName)
		{
			/*
			 * The client has not set a connection.
			 */
			if (id == Guid.Empty)
				throw RuntimeException.ConnectionNotSet(context, CreateDescriptor(RuntimeEvents.CreateConnection, sender.Authority, sender.Id, sender.Property, sender.MicroService), authorityName);
			/*
			 * The client has invalid connection set. This is probably due to the deleted connection or misbehaved import.
			 */
			if (!(context.GetServerContext().GetService<IComponentService>().SelectConfiguration(id) is IConnection connection))
				throw RuntimeException.ConnectionNotFound(context, CreateDescriptor(RuntimeEvents.CreateConnection, sender.Authority, sender.Id, sender.Property, sender.MicroService), authorityName);
			/*
			 * We don't allow disabled connections to execute
			 */
			if (!connection.Enabled)
				throw RuntimeException.Create(context, string.Format("{0} ({1}, {2}).", SR.ErrConnectionDisabled, authorityName, connection.ComponentName(context)), CreateDescriptor(RuntimeEvents.CreateConnection, sender.Authority, sender.Id, sender.Property, sender.MicroService));
		...
		protected IDataProvider CreateDataProvider(IApplicationContext context, IExecutionContext sender, IConnection connection)
		{
			/*
			 * Connection is not properly configured. We just notify the user about the issue.
			 */
			if (connection.DataProvider == Guid.Empty)
				throw RuntimeException.ConnectionDataProviderNotSet(context, CreateDescriptor(sender.Event, ExecutionContext.Connection, connection.Component.ToString(), null, sender.MicroService), connection.ComponentName(context));

			var provider = context.GetServerContext().GetService<IDataProviderService>().Select(connection.DataProvider);
			/*
			 ...
			 */
			if (provider == null)
				throw RuntimeException.ConnectionDataProviderNotFound(Context, CreateDescriptor(sender.Event, ExecutionContext.Connection, connection.Component.ToString(), null, sender.MicroService), connection.ComponentName(Context));
		...
		protected bool IsDefined(IExecutionContext sender, string name, List<IDataParameter> parameters)
		{
			if (string.IsNullOrWhiteSpace(name))
				throw RuntimeException.ParameterNameNotSet(Context, CreateDescriptor(0));
			/*
		...
		protected bool MatchArgument(IExecutionContext sender, string name, JObject arguments, out object value)
		{
			value = null;

			if (string.IsNullOrWhiteSpace(name))
				throw RuntimeException.ParameterNameNotSet(Context, sender);
			/*
		...
		protected bool Value(IExecutionContext sender, IDataParameter parameter, object proposedValue, out object value, out System.Data.ParameterDirection direction)
		{
			/*
			...
			if (proposedValue == null || value == DBNull.Value)
			{
				if (!parameter.IsNullable)
					throw RuntimeException.ParameterExpected(Context, sender, parameter.Name);
				/*
			...
			if (!Types.TryConvert(value, out object converted, type))
				throw RuntimeException.ParameterConversion(Context, sender, parameter.Name, value.ToString(), type.Name);
			/*
		...
		private object ProcessTimezone(IExecutionContext sender, IDataParameter parameter, object proposedValue)
		{
			/*
			...
			if (parameter.DataType != DataType.Date)
				throw RuntimeException.TimezoneParametersSupportedOnDatesOnly(Context, sender, parameter.Name);
			/*
		...
		protected void ValidateParameterDatatype(IExecutionContext sender, IDataParameter parameter, object value)
		{
			/*
			...
			throw RuntimeException.InvalidParameterDataType(Context, sender, parameter.Name, value.ToString(), parameter.DataType.ToString());
		}
		/// <summary>
		...
		protected void ValidateArguments(IExecutionContext sender, List<IDataParameter> parameters, JObject arguments)
		{
			if (arguments == null)
				return;
			...
			foreach (var i in arguments)
			{
				if (!IsDefined(sender, i.Key, parameters))
					throw RuntimeException.ParameterNotDefined(Context, sender, i.Key);
			}
		}

		protected void SetCommandParameters(IExecutionContext sender, IDataCommandDescriptor command, List<IDataParameter> configuration, JObject arguments)
		{
			...
			foreach (var i in configuration)
			{
				...
				{
					if (!i.IsNullable)
						throw RuntimeException.ParameterValueNotSet(Context, sender, i.Name);
				}
			}
		}
	}
