...
namespace TomPIT.Runtime.ApplicationContextServices
{
	...
	{
		public ValidationService(IApplicationContext context)
		{
			Context = context;
		}
		...
		public void Exists(string attribute, object value, List<object> values)
		{
			if (values == null || values.Count == 0)
				throw new ApiException(SR.SourceValidation, string.Format(SR.ValNotExists, value, attribute));
		...
		public void MaxLength(string attribute, string value, int length)
		{
			...
			if (value.Length > length)
				throw new ApiException(SR.SourceValidation, string.Format(SR.ValMaxLength, length, attribute));
		}

		public void MaxValue(string attribute, double value, double maxValue)
		{
			if (value > maxValue)
				throw new ApiException(SR.SourceValidation, string.Format(string.Format("{0} ({1})", string.Format(SR.ValMaxValue, maxValue), attribute)));
		}
		...
		public void MinLength(string attribute, string value, int length)
		{
			...
			if (value.Length < length)
				throw new ApiException(SR.SourceValidation, string.Format(SR.ValMinLength, length, attribute));
		}

		public void MinValue(string attribute, double value, double minValue)
		{
			if (value < minValue)
				throw new ApiException(SR.SourceValidation, string.Format(string.Format("{0} ({1})", string.Format(SR.ValMinValue, minValue), attribute)));
		}
		...
		public void Range(string attribute, double value, double minValue, double maxValue)
		{
			if (value < minValue || value > maxValue)
				throw new ApiException(SR.SourceValidation, string.Format(SR.ValRange, minValue, maxValue, attribute));
		}
		...
		{
			if (string.IsNullOrWhiteSpace(value))
				throw new ApiException(SR.SourceValidation, string.Format(SR.ValRequired, attribute));
		}
		...
		public void Unique(string attribute, object value, string keyProperty, object keyValue, JObject existing)
		{
			...
			foreach (var i in tokens)
			{
				...
				throw new ApiException(SR.SourceValidation, string.Format("{0}. ({1})", SR.ValExists, attribute));
			}
		}
	}
}
