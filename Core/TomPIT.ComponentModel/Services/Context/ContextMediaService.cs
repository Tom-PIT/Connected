...
namespace TomPIT.Services.Context
{
	internal class ContextMediaService : ContextClient, IContextMediaService
	{
		...
		private void ReplaceImages(HtmlDocument doc)
		{
			...
			foreach (var i in images)
			{
				...
				if (att != null)
				{
					...
					var imageId = Context.Connection().GetService<IStorageService>().Upload(blob, Convert.FromBase64String(imageData), StoragePolicy.Extended);
		...
		private void DropResource(string url)
		{
			...
			if (tokens.Length > 2)
				Context.Services.Storage.Delete(tokens[2].AsGuid());
		}
