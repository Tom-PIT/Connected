...
namespace TomPIT.Data.Sql
{
	...
	internal class RetryManager
	{
		...
		public static void SetDefault(RetryManager retryManager, bool throwIfSet = true)
		{
			...
			{
				throw new InvalidOperationException(SR.ExceptionRetryManagerAlreadySet);
			}
		...
		public static RetryManager Instance
		{
			get
			{
				...
				if (instance == null)
				{
					throw new InvalidOperationException(SR.ExceptionRetryManagerNotSet);
				}
				...
				foreach (var map in this.defaultRetryStrategyNamesMap.Where(x => !string.IsNullOrWhiteSpace(x.Value)))
				{
					...
					if (!this.retryStrategies.TryGetValue(map.Value, out strategy))
					{
						throw new ArgumentOutOfRangeException(
							 ...
							 string.Format(CultureInfo.CurrentCulture, SR.DefaultRetryStrategyMappingNotFound, map.Key, map.Value));
					}
		...
		public string DefaultRetryStrategyName
		{
			...
			set
			{
				if (!string.IsNullOrWhiteSpace(value))
				{
					...
					{
						throw new ArgumentOutOfRangeException("value", string.Format(CultureInfo.CurrentCulture, SR.RetryStrategyNotFound, value));
					}
				}
				else
				{
		...
		public virtual RetryStrategy GetRetryStrategy(string retryStrategyName)
		{
			...
			{
				throw new ArgumentOutOfRangeException(string.Format(CultureInfo.CurrentCulture, SR.RetryStrategyNotFound, retryStrategyName));
			}
		...
		public virtual RetryStrategy GetDefaultRetryStrategy(string technology)
		{
			...
			{
				throw new ArgumentOutOfRangeException(string.Format(CultureInfo.CurrentCulture, SR.DefaultRetryStrategyNotFound, technology));
			}
