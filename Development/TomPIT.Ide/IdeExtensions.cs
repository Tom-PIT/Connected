...
namespace TomPIT.Ide
{
	public static class IdeExtensions
	{
		...
		public static IElement Find(this IConfiguration configuration, Guid id)
		{
			...
			{
				var target = Find(configuration, i, id, refs);
		...
		private static IElement Find(object instance, Guid id, List<object> refs)
		{
			var props = DomQuery.Properties(instance, false, false);

			foreach (var i in props)
			{
				var target = Find(instance, i, id, refs);
		...
		private static IElement Find(object configuration, PropertyInfo property, Guid id, List<object> refs)
		{
			...
			if (property.IsCollection())
			{
				...
				while (enm.MoveNext())
				{
					...
					var target = Find(enm.Current, id, refs);
			...
			{
				if (value is IElement element && element.Id == id)
					return element;

				var target = Find(value, id, refs);
