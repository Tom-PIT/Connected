...
namespace TomPIT.Ide.TextEditor.CSharp.Services.CompletionProviders
{
	internal class ScriptReferenceProvider : CompletionProvider
	{
		protected override List<ICompletionItem> OnProvideItems()
		{
			...
			{
				FillItems(result, Editor.MicroService);

				var refs = Editor.Context.Tenant.GetService<IDiscoveryService>().References(Editor.MicroService.Token);
		...
		{
			var msName = microService.Token == Editor.MicroService.Token ? string.Empty : $"{microService.Name}/";
