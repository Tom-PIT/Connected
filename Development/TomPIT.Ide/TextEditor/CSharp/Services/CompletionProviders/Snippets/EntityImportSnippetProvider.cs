...
namespace TomPIT.Ide.TextEditor.CSharp.Services.CompletionProviders.Snippets
{
	internal class EntityImportSnippetProvider : SnippetProvider
	{
		...
		{
			var connections = Editor.Context.Tenant.GetService<IComponentService>().QueryComponents(Editor.MicroService.Token, ComponentCategories.Connection);
			var r = new List<ICompletionItem>();
			...
			foreach (var connection in connections)
			{
				r.Add(new CompletionItem
				{
					...
					Kind = CompletionItemKind.Text,
					InsertText = $"\"{Editor.MicroService.Name}/{connection.Name}\"",
