...
using TomPIT.ComponentModel.Analysis;
...
namespace TomPIT.Development.CodeAnalysis.SnippetProviders
{
	internal class EntityImportProvider : ISnippetProvider
	{
		...
		{
			var connections = e.Context.Connection().GetService<IComponentService>().QueryComponents(e.Context.MicroService.Token, "Connection");
			var r = new List<ISuggestion>();
		...
		private (ISchemaBrowser, IConnection) ResolveSchemaBrowser(SnippetArgs e, string name)
		{
			if (string.IsNullOrWhiteSpace(name))
				return (null, null);

			var tokens = name.Split("/");
			var ms = e.Context.Connection().GetService<IMicroServiceService>().Select(tokens[0]);

			if (ms == null)
				return (null, null);

			var connection = e.Context.Connection().GetService<IComponentService>().SelectConfiguration(ms.Token, "Connection", tokens[1]) as IConnection;

			if (connection == null || connection.DataProvider == Guid.Empty)
				return (null, null);

			var provider = e.Context.Connection().GetService<IDataProviderService>().Select(connection.DataProvider);
