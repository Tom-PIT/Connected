...
namespace TomPIT.Development.Analysis.Providers
{
	internal class EventProvider : CodeAnalysisProvider
	{
		...
		public EventProvider(IMiddlewareContext context) : base(context)
		{
		...
		public override List<ICodeAnalysisResult> ProvideLiterals(IMiddlewareContext context, CodeAnalysisArgs e)
		{
			...
			var ds = context.Tenant.GetService<IComponentService>().QueryComponents(e.Component.MicroService, "Event");
			...
			foreach (var i in ds)
			{
				if (!(context.Tenant.GetService<IComponentService>().SelectConfiguration(i.Token) is IDistributedEventConfiguration config))
					continue;
		...
		private void AddReferences(IMiddlewareContext context, Guid microService, string existingText, List<ICodeAnalysisResult> existing)
		{
			...
			foreach (var i in refs.MicroServices)
			{
				...
				var ds = context.Tenant.GetService<IComponentService>().QueryComponents(ms.Token, "Event");

				foreach (var j in ds)
				{
					if (!(context.Tenant.GetService<IComponentService>().SelectConfiguration(j.Token) is IEvent config))
						continue;
		...
		{
			return context.Tenant.GetService<IComponentService>().SelectComponent(microService, "Event", qualifier);
		}
	}
}
