...
public class AssemblyResource : TextConfiguration, IAssemblyResourceConfiguration
{
	...
	public async Task ProcessDeleted()
	{
		...
		foreach (var blob in blobs)
			Tenant.GetService<IDesignService>().Components.Delete(ms.Token, blob, BlobTypes.AutoGenerated);
	...
	private async Task UpdateFile()
	{
		...
		Tenant.GetService<IDesignService>().Components.Update(ms.Token, Component, BlobTypes.AutoGenerated, "plain/text", $"{ToString()}.Designer.cs", Component.ToString(), Encoding.UTF8.GetBytes(text));
	}
}
