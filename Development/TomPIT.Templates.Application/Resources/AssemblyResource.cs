...
public class AssemblyResource : TextConfiguration, IAssemblyResourceConfiguration
{
	...
	private async Task UpdateFile()
	{
		...
		Tenant.GetService<IDesignService>().Components.Update(ms.Token, TextBlob, BlobTypes.AutoGenerated, "plain/text", $"{ToString()}.Designer.cs", Component.ToString(), Encoding.UTF8.GetBytes(text));
	}
}
