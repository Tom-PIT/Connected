...
namespace TomPIT.Sys.Data
{
    public class Deployment
    {
        ...
        {
            var u = new MarketplaceUrl("IPackages", "Delete");
            var e = new JObject
                {
        ...
        public List<IPublishedPackage> QueryPublicPackages()
        {
            ...
            return new HttpConnection().Get<List<PublishedPackage>>(u, new HttpRequestArgs().WithBasicCredentials(UserName, Password)).ToList<IPublishedPackage>();
        }
        ...
        internal List<Guid> CheckForUpdates(List<PackageVersion> packages)
        {
            ...
            var r = new HttpConnection().Post<JObject>(u, e);
            var resultArray = r.Optional<JArray>("packages", null);
            var result = new List<Guid>();

            if (resultArray == null)
                return result;

            foreach (JValue value in resultArray)
                result.Add(value.Value<Guid>());
