...
namespace TomPIT.Sys.Model.Blobs
{
	public class BlobsModel : CacheRepository<IBlob, Guid>
	{
		...
		public void Commit(string draft, string primaryKey)
		{
			...
			//TODO
			//Implement file type storage, so we can check for singleton overwrites here.
			 ...
			 StoragePolicy policy, Guid token)
		{
			...
			if (isDraft || policy == StoragePolicy.Extended)
			{
				...
				//Do not allow more than one file per singleton blob, even if names mismatch
				if (isDraft && policy == StoragePolicy.Singleton)
					existingDraft ??= existingDrafts.FirstOrDefault();

				if (existingDraft == null)
					return Insert(token, resourceGroup, type, primaryKey, microService, topic, fileName, contentType, draft, content);
				else
				{
					if (string.Compare(existingDraft.Topic, topic, true) != 0)
						throw new SysException(SR.ErrTopicMismatch);
		...
		public void RefreshBlob(Guid token)
		{
			var existing = Select(token);
			Refresh(token);

			if (existing is not null)
				DataModel.BlobsContents.Refresh(existing.ResourceGroup, existing.Token);
		}
	}
}
