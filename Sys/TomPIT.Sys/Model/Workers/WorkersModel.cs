...
{
   internal class WorkersModel : SynchronizedRepository<ISysScheduledJob, Guid>
   {
      ...
      {
         if (DataModel.Queue.Select(popReceipt) is IQueueMessage m)
            DataModel.Queue.Complete(popReceipt);

         var w = Select(worker);
         var status = WorkerStatus.Enabled;
         var elapsed = DateTime.UtcNow.Subtract(m is null ? DateTime.UtcNow : m.DequeueTimestamp).TotalMilliseconds;
