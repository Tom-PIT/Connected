...
namespace TomPIT.Sys.Search
{
	...
	{
		public TermSearchJob(ISearchOptions options) : base(options)
		{
		}
		...
		protected override string ParseCommandText()
		{
			...
			sb.AppendFormat("({0}:{1}", SearchFields.Title, CommandText);
			sb.AppendFormat(" OR {0}:{1}", SearchFields.Content, CommandText);
			sb.AppendFormat(" OR {0}:{1}", SearchFields.Tags, CommandText);
			sb.AppendFormat(" OR {0}:{1}", SearchFields.ComponentName, CommandText);
			sb.AppendFormat(" OR {0}:{1}", SearchFields.ElementName, CommandText);
			sb.Append(")");
