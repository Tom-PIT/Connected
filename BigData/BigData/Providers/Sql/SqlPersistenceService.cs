...
namespace TomPIT.BigData.Providers.Sql
{
	internal class SqlPersistenceService : MiddlewareObject, IPersistenceService
	{
		...
		{
			using var w = new NodeReader<MergeResultRecord>(node, CreateCommandText(provider, context, true), CommandType.Text);
		...
		private static void UpdateFieldStatistics(INode node, IPartitionFile file, PartitionSchemaField field)
		{
			...
			{
				using var reader = new NodeReader<FileFieldStatistics<string>>(node, commandText, CommandType.Text);
				var result = reader.ExecuteSingleRow();

				startString = result.Min;
				endString = result.Max;
			}
			else if (field is PartitionSchemaDateField)
			{
				using var reader = new NodeReader<FileFieldStatistics<DateTime>>(node, commandText, CommandType.Text);
				var r = reader.ExecuteSingleRow();
			...
			else if (field is PartitionSchemaNumberField nf)
			{
				...
				{
					using var reader = new NodeReader<FileFieldStatistics<byte>>(node, commandText, CommandType.Text);
					var b = reader.ExecuteSingleRow();
				...
				{
					using var reader = new NodeReader<FileFieldStatistics<short>>(node, commandText, CommandType.Text);
					var s = reader.ExecuteSingleRow();
				...
				{
					using var reader = new NodeReader<FileFieldStatistics<int>>(node, commandText, CommandType.Text);
					var i = reader.ExecuteSingleRow();
				...
				{
					using var reader = new NodeReader<FileFieldStatistics<long>>(node, commandText, CommandType.Text);
					var l = reader.ExecuteSingleRow();
				...
				{
					using var reader = new NodeReader<FileFieldStatistics<double>>(node, commandText, CommandType.Text);
					var f = reader.ExecuteSingleRow();
