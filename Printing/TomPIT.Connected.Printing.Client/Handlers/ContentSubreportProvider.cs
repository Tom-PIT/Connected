...
{
	internal class ContentSubreportProviderAsync : IReportProviderAsync
	{
		private string _content;
		Dictionary<string, MemoryStream> _subreports = new Dictionary<string, MemoryStream>();

		public ContentSubreportProviderAsync(string completeContent)
		{
	...
	internal class ContentSubreportProvider(string completeContent) : IReportProvider
	{
		Dictionary<string, MemoryStream> _subreports = ParseSubreports(completeContent);

		public XtraReport GetReport(string reportUrl, ReportProviderContext context)
		{
			if (_subreports.GetValueOrDefault(reportUrl, null) is not MemoryStream ms)
				return null;
